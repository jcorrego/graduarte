...
define('SCRIPT_FILENAME',	isset($_SERVER['SCRIPT_FILENAME']) ? $_SERVER['SCRIPT_FILENAME'] : '');
...
if (!file_exists($docRoot . $image))
{
	...
	$image = preg_replace('/~(\w+)\//','',$image);
	$scriptName = preg_replace('/~(\w+)\//','',SCRIPT_NAME);
	// Build doc root from other server variables
	$docRoot = str_replace($scriptName,'',SCRIPT_FILENAME);
