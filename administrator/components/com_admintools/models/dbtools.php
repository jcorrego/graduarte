...
class AdmintoolsModelDbtools extends JModel
{
	...
	public function repairAndOptimise($fromTable = null)
	{
		...
		while(count($tables) && $this->haveEnoughTime())
		{
			...
			$db->setQuery('CHECK TABLE '.$db->nameQuote($table));
			...
			if(!$isOK)
			{
				...
				$db->setQuery('REPAIR TABLE '.$db->nameQuote($table));
				$db->query();
			}

			// Finally, optimize
			$db->setQuery('OPTIMIZE TABLE '.$db->nameQuote($table));
			$db->query();
		}
	...
	public function purgeSessions()
	{
		...
		$db->setQuery('TRUNCATE TABLE '.$db->nameQuote('#__session'));
		$db->query();

		$db->setQuery('DELETE FROM '.$db->nameQuote('#__session'));
		$db->query();
