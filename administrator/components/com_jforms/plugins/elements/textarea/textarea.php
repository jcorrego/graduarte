...
*
* @version		$Id: textarea.php 306 2009-09-07 06:44:45Z dr_drsh $
* @package		Joomla
* @subpackage	JForms
...
class JFormEPluginTextarea{
	...
	function render( $elementData ){
		...
		$rows = floor( $p->ch / 22.5 );
		$cols = floor( $p->cw / 0.05 );
		...
		$output .= _line("<textarea rows='$rows' cols='$cols' class='$inputClass' name='$p->hash' id='$htmlId' style='width:{$p->cw}px;height:{$p->ch}px;'>$default</textarea>",2);
